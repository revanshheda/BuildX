...
type ApprovalNodeData = { approval: ApprovalGraphNode; status: ApprovalGraphStatus; applicationStatus?: string; selected: boolean; onSelect: (id: string) => void };
type StageNodeData = { stage: ApprovalGraphStage };

const STATUS_META: Record<ApprovalGraphStatus, { label: string; icon: React.ReactNode }> = {
...
export default function PersonalizedRoadmapPage() {
  ...
  const readyCount = Object.values(statuses).filter((status) => status === 'READY_TO_APPLY').length;
  const lockedCount = Object.values(statuses).filter((status) => status === 'LOCKED').length;
  const selectNode = (id: string) => { setSelectedId(id); setDrawerOpen(true); };
  const flowNodes = useMemo(() => [...stageNodes, ...MAHARASHTRA_COLD_STORAGE_NODES.map((approval) => ({ id: approval.id, type: 'approval' as const, position: graphPositions[approval.id], data: { approval, status: statuses[approval.id], selected: approval.id === selectedId, onSelect: selectNode, applicationStatus: approval.applicationLinked ? application.status : undefined } }))], [application.status, selectedId, statuses]);
